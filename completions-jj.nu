module completions {
  ...
  export extern "jj branch create" [
    ...
    --to: string              # The branch's target revision
  ...
  export extern "jj branch list" [
    --all-remotes(-a)         # Show all tracking and non-tracking remote branches including the ones whose targets are synchronized with the local branches
    --tracked(-t)             # Show remote tracked branches only. Omits local Git-tracking branches by default
    --conflicted(-c)          # Show conflicted branches only
    ...
    --template(-T): string    # Render each branch using the given template
    --repository(-R): string  # Path to repository to operate on
    --ignore-working-copy     # Don't snapshot the working copy, and don't update it
    --ignore-immutable        # Allow rewriting immutable commits
    --at-operation: string    # Operation to load the repo at
  ...
  export extern "jj branch move" [
    --from: string            # Move branches from the given revisions
    --to: string              # Move branches to this revision
    --allow-backwards(-B)     # Allow moving branches backwards or sideways
    ...names: string          # Move branches matching the given name patterns
    --repository(-R): string  # Path to repository to operate on
    --ignore-working-copy     # Don't snapshot the working copy, and don't update it
    --ignore-immutable        # Allow rewriting immutable commits
    --at-operation: string    # Operation to load the repo at
  ...
  export extern "jj log" [
    --revisions(-r): string   # Which revisions to show. If no paths nor revisions are specified, this defaults to the `revsets.log` setting, or `@ | ancestors(immutable_heads().., 2) | trunk()` if it is not set
